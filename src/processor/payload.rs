...
// use regex::Regex;
use protobuf::{Message, parse_from_bytes, ProtobufError};
...
impl ArcherPayload {
    pub fn new(payload_data: &[u8]) -> Result<ArcherPayload, ApplyError> {
        ...
        let payload: PayloadPB = match payload {
            Ok(pl) => pl,
            Err(_) => {
                return Err(ApplyError::InvalidTransaction(String::from("Invalid payload serialization")))
            }
        };
    ...
    pub fn data(&self) -> Result<Vec<u8>, ApplyError> {
        match self.payload.get_action() {
            Payload_Action::DEPOSIT => {
                if self.payload.descriptor().get_field_by_name("amount").is_some() {
                    Ok(self.payload.write_to_bytes().expect("Error converting action message to bytes"))
                } else {
                    Err(ApplyError::InvalidTransaction(String::from("Action does not match payload data")))
                }
            },
            ...
            Payload_Action::UPDATE_NUMBER => {
                if self.payload.descriptor().get_field_by_name("new_number").is_some() {
                    Ok(self.payload.write_to_bytes().expect("Error converting action message to bytes"))
                } else {
                    Err(ApplyError::InvalidTransaction(String::from("Action does not match payload data")))
                }
            },
            Payload_Action::ADD_ACCOUNT => Ok(
                self.payload.write_to_bytes().expect("Error converting action message to bytes")
            ),
        }
    }
