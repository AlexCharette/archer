...
use crate::protobuf::account::{AccountContainer, Account as AccountPB};
use crate::protobuf::merchant::{MerchantContainer, Merchant as MerchantPB};
...
impl<'a> ArcherState<'a> {
    ...
    */
    ...
    pub fn set_merchant(&mut self, public_key: &str, number: u32) -> Result<(), ApplyError> {
        let address: String = calculate_merchant_address(public_key);
        let mut account: MerchantPB = MerchantPB::new();
        account.set_name(String::from(public_key));
        account.set_number(number);
        account.set_balance(0);
    ...
    pub fn update_number(&mut self, name: &str, number: u32, new_number: u32) -> Result<u32, ApplyError> {
        let address: String = calculate_account_address(name);
        let state_entries = self.context.get_state_entries(&vec![String::from(address.clone())]);
        match state_entries {
            Ok(entries) => {
                let container: AccountContainer = parse_from_bytes(&entries[0].1).expect("Error parsing state entries");
                let mut accounts = container.get_entries().to_vec();
                let account: &mut AccountPB = accounts.iter_mut().filter(|entry|
                    entry.get_name() == name && entry.get_number() == number
                ).next().expect("Did not found accounts with that name");
                account.set_number(new_number);
                Ok(account.get_number())
            },
            Err(_) => {
                return Err(ApplyError::InvalidTransaction(String::from(
                    format!("Account not found for {}", address)
                )))
            },
        }
    }
    ...
    pub fn get_balance(&mut self, name: &str, number: u32) -> Result<i32, ApplyError> {
        ...
        match state_entries {
            Ok(entries) => {
                let container: AccountContainer = parse_from_bytes(&entries[0].1).expect("Error parsing state entries");
                let account: &AccountPB = container.entries.iter().filter(|entry|
                    entry.get_name() == name && entry.get_number() == number
                ).next().expect("Did not found accounts with that name");
    ...
    pub fn update_balance(&mut self, name: &str, number: u32, amount: i32) -> Result<i32, ApplyError> {
        let address: String = calculate_account_address(name);
    ...
    fn _update_balance(&mut self, address: &str, name: &str, number: u32, amount: i32) -> Result<(), ApplyError> {
        ...
        match state_entries {
            Ok(entries) => {
                ...
                // updated_state[address] = data
                // self._context.set_state(updated_state, timeout=self._timeout)
                let data = container.write_to_bytes().expect("Could not serialize container");
