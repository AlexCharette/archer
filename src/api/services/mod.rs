...
pub fn make_deposit_txn<'a>(
    txn_signer: &'a Signer, batch_signer: &'a Signer, 
    name: String, number: u32, amount: i32
) -> (Vec<u8>, String) {
    ...
    let payload_bytes = payload_pb.write_to_bytes().expect("Error converting protobuf payload to bytes");
