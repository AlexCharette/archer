...
pub fn make_batch<'a>(
    ...
    // create txn header
    let txn_header = transaction::make_header(&payload_bytes, inputs, outputs, txn_signer, batch_signer);
    let txn_header_bytes = txn_header.write_to_bytes().expect("Error converting transaction header to bytes");

    // create txn 
    let txn_header_signature = txn_signer.sign(&txn_header_bytes)
        .expect("Error signing transaction header bytes");
    let transaction = transaction::make_transaction(&payload_bytes, &txn_header_bytes, &txn_header_signature);
    let transactions = vec![&transaction];

    // create batch header
    let batch_header = make_header(&transactions, batch_signer);
    let batch_header_bytes = batch_header.write_to_bytes().expect("Error converting batch header to bytes");

    let batch_header_signature = batch_signer.sign(&batch_header_bytes)
